#define SERIAL_DEBUG_ENABLED 0    //<  1 for serial debug informations

#define DATASIZE DATASIZE_PRTCL + DATASIZE_FLOAT + DATASIZE_FLOAT + DATASIZE_FLOAT ///< Protocolheader + temperature + turbidity + ph
...
#define PRTCL_PH      0x0006 ///< id of ph is 6
...
//defines for ph
#define PH_SENSOR_PIN A1            //pH meter Analog output to Arduino Analog Input 1
#define PH_OFFSET -0.036           //deviation compensate
...
void build_msg()
{
    ...
    FtoLE(temp, &mydata[9]);
    mydata[13] = (PRTCL_PH & 0xFF);
    mydata[14] = (PRTCL_PH >> 8);
    FtoLE(ph, &mydata[15]);
}
...
/**
 * @brief  Initialises the ph-sensor.
 * @details The used sensor doesn't need any setup as it always sends a value as a voltage. This voltage will be converted by read_ph().
 */
void setup_ph()
{
}
...
float read_temp(void)
{
  ...
  temp = temp_sensors.readTemperature(temp_sensor_adress) + 273.15;  //°C to K
...
float read_turb()
{
  ...
  DebugPrint("turbidity: ");
  DebugPrintln(sensorValue,2);
  return sensorValue * (5.0 / 1024.0); // Convert the analog reading (which goes from 0 - 1023) to a voltage (0 - 5V):
}

/**
 * @brief  Reads the current (absolure) ph value.
 * @details Be sure to properly calibrate the sensor; set PH_OFFSET accordingly.
 * @return Turbidity (relative) in Volts
 */
float read_ph()
{
  float voltage, phValue;
  voltage = analogRead(PH_SENSOR_PIN) * (5.0/1024); // Convert the analog reading (which goes from 0 - 1023) to a voltage (0 - 5V):
  phValue = 3.5*voltage+PH_OFFSET; //Conver the voltage to ph
  DebugPrint("phValue: ");
  DebugPrintln(phValue,2);
  return phValue;
}
